...
router.post('/create-transactions', verifyToken, async(req, res, next) => {
    let banks = [],
        statusDetail
    const loggedUserAccount = await accountModel.findOne({ user: req.userId});
    console.log(loggedUserAccount.accountNumber);
    ...
    const transaction = transactionModel.create({
        ...
        accountFrom: loggedUserAccount.accountNumber,
